-- -*- haskell -*- File generated by the BNF Converter (bnfc 2.9.6.3).

-- Parser definition for use with Happy
{
{-# OPTIONS_GHC -Wno-incomplete-patterns -Wno-overlapping-patterns #-}
{-# LANGUAGE PatternSynonyms #-}

module ParCPP
  ( happyError
  , myLexer
  , pProgram
  , pDef
  , pListDef
  , pArg
  , pType
  , pListArg
  , pStm
  , pListStm
  , pExp15
  , pExp14
  , pExp13
  , pExp12
  , pExp11
  , pExp10
  , pExp9
  , pExp8
  , pExp4
  , pExp3
  , pExp2
  , pExp
  , pExp1
  , pExp5
  , pExp6
  , pExp7
  , pListExp
  , pListId
  ) where

import Prelude

import qualified AbsCPP
import LexCPP

}

%name pProgram Program
%name pDef Def
%name pListDef ListDef
%name pArg Arg
%name pType Type
%name pListArg ListArg
%name pStm Stm
%name pListStm ListStm
%name pExp15 Exp15
%name pExp14 Exp14
%name pExp13 Exp13
%name pExp12 Exp12
%name pExp11 Exp11
%name pExp10 Exp10
%name pExp9 Exp9
%name pExp8 Exp8
%name pExp4 Exp4
%name pExp3 Exp3
%name pExp2 Exp2
%name pExp Exp
%name pExp1 Exp1
%name pExp5 Exp5
%name pExp6 Exp6
%name pExp7 Exp7
%name pListExp ListExp
%name pListId ListId
-- no lexer declaration
%monad { Err } { (>>=) } { return }
%tokentype {Token}
%token
  '!'       { PT _ (TS _ 1)  }
  '!='      { PT _ (TS _ 2)  }
  '%'       { PT _ (TS _ 3)  }
  '&'       { PT _ (TS _ 4)  }
  '&&'      { PT _ (TS _ 5)  }
  '('       { PT _ (TS _ 6)  }
  ')'       { PT _ (TS _ 7)  }
  '*'       { PT _ (TS _ 8)  }
  '+'       { PT _ (TS _ 9)  }
  '++'      { PT _ (TS _ 10) }
  ','       { PT _ (TS _ 11) }
  '-'       { PT _ (TS _ 12) }
  '--'      { PT _ (TS _ 13) }
  '->'      { PT _ (TS _ 14) }
  '.'       { PT _ (TS _ 15) }
  '/'       { PT _ (TS _ 16) }
  ':'       { PT _ (TS _ 17) }
  ';'       { PT _ (TS _ 18) }
  '<'       { PT _ (TS _ 19) }
  '<<'      { PT _ (TS _ 20) }
  '<='      { PT _ (TS _ 21) }
  '='       { PT _ (TS _ 22) }
  '=='      { PT _ (TS _ 23) }
  '>'       { PT _ (TS _ 24) }
  '>='      { PT _ (TS _ 25) }
  '>>'      { PT _ (TS _ 26) }
  '?'       { PT _ (TS _ 27) }
  '['       { PT _ (TS _ 28) }
  ']'       { PT _ (TS _ 29) }
  'bool'    { PT _ (TS _ 30) }
  'const'   { PT _ (TS _ 31) }
  'double'  { PT _ (TS _ 32) }
  'else'    { PT _ (TS _ 33) }
  'false'   { PT _ (TS _ 34) }
  'for'     { PT _ (TS _ 35) }
  'if'      { PT _ (TS _ 36) }
  'int'     { PT _ (TS _ 37) }
  'return'  { PT _ (TS _ 38) }
  'string'  { PT _ (TS _ 39) }
  'throw'   { PT _ (TS _ 40) }
  'true'    { PT _ (TS _ 41) }
  'typedef' { PT _ (TS _ 42) }
  'vector'  { PT _ (TS _ 43) }
  'void'    { PT _ (TS _ 44) }
  'while'   { PT _ (TS _ 45) }
  '{'       { PT _ (TS _ 46) }
  '||'      { PT _ (TS _ 47) }
  '}'       { PT _ (TS _ 48) }
  L_charac  { PT _ (TC $$)   }
  L_doubl   { PT _ (TD $$)   }
  L_integ   { PT _ (TI $$)   }
  L_quoted  { PT _ (TL $$)   }
  L_Id      { PT _ (T_Id $$) }

%%

Char    :: { Char }
Char     : L_charac { (read $1) :: Char }

Double  :: { Double }
Double   : L_doubl  { (read $1) :: Double }

Integer :: { Integer }
Integer  : L_integ  { (read $1) :: Integer }

String  :: { String }
String   : L_quoted { $1 }

Id :: { AbsCPP.Id }
Id  : L_Id { AbsCPP.Id $1 }

Program :: { AbsCPP.Program }
Program : ListDef { AbsCPP.PDefs $1 }

Def :: { AbsCPP.Def }
Def
  : Type Id '(' ListArg ')' '{' ListStm '}' { AbsCPP.DFun $1 $2 $4 $7 }
  | Type Id '(' ListArg ')' ';' { AbsCPP.DDecl $1 $2 $4 }
  | 'typedef' Type Id ';' { AbsCPP.DTypedef $2 $3 }

ListDef :: { [AbsCPP.Def] }
ListDef : {- empty -} { [] } | Def ListDef { (:) $1 $2 }

Arg :: { AbsCPP.Arg }
Arg : Type Id { AbsCPP.ADecl $1 $2 } | Type { AbsCPP.ADeclNoId $1 }

Type :: { AbsCPP.Type }
Type
  : 'const' Type { AbsCPP.TConst $2 }
  | Id { AbsCPP.TId $1 }
  | Type '&' { AbsCPP.TRef $1 }
  | 'bool' { AbsCPP.Type_bool }
  | 'int' { AbsCPP.Type_int }
  | 'double' { AbsCPP.Type_double }
  | 'void' { AbsCPP.Type_void }
  | 'string' { AbsCPP.Type_string }
  | 'vector' { AbsCPP.Type_vector }

ListArg :: { [AbsCPP.Arg] }
ListArg
  : {- empty -} { [] }
  | Arg { (:[]) $1 }
  | Arg ',' ListArg { (:) $1 $3 }

Stm :: { AbsCPP.Stm }
Stm
  : Exp ';' { AbsCPP.SExp $1 }
  | Type ListId ';' { AbsCPP.SDecls $1 $2 }
  | Type Id '=' Exp ';' { AbsCPP.SInit $1 $2 $4 }
  | 'return' Exp ';' { AbsCPP.SReturn $2 }
  | 'return' ';' { AbsCPP.SReturnVoid }
  | 'while' '(' Exp ')' Stm { AbsCPP.SWhile $3 $5 }
  | 'for' '(' Stm Exp ';' Exp ')' Stm { AbsCPP.SFor $3 $4 $6 $8 }
  | '{' ListStm '}' { AbsCPP.SBlock $2 }
  | 'if' '(' Exp ')' Stm { AbsCPP.SIf $3 $5 }
  | 'if' '(' Exp ')' Stm 'else' Stm { AbsCPP.SIfElse $3 $5 $7 }
  | 'throw' Exp ';' { AbsCPP.SThrow $2 }
  | 'typedef' Type Id ';' { AbsCPP.STypedef $2 $3 }
  | ';' { AbsCPP.SEmpty }

ListStm :: { [AbsCPP.Stm] }
ListStm : {- empty -} { [] } | Stm ListStm { (:) $1 $2 }

Exp15 :: { AbsCPP.Exp }
Exp15
  : 'true' { AbsCPP.ETrue }
  | 'false' { AbsCPP.EFalse }
  | Integer { AbsCPP.EInt $1 }
  | Double { AbsCPP.EDouble $1 }
  | String { AbsCPP.EString $1 }
  | Char { AbsCPP.EChar $1 }
  | Id { AbsCPP.EId $1 }
  | Id '(' ListExp ')' { AbsCPP.EApp $1 $3 }
  | Exp15 '.' Id { AbsCPP.EDot $1 $3 }
  | Exp15 '->' Id { AbsCPP.EArrow $1 $3 }
  | Exp15 '(' ListExp ')' { AbsCPP.EAppExp $1 $3 }
  | Exp15 String { AbsCPP.EStringConcat $1 $2 }
  | Exp15 '[' Exp ']' { AbsCPP.EIndex $1 $3 }
  | '(' Exp ')' { $2 }

Exp14 :: { AbsCPP.Exp }
Exp14
  : Exp15 '++' { AbsCPP.EPIncr $1 }
  | Exp15 '--' { AbsCPP.EPDecr $1 }
  | Exp15 { $1 }

Exp13 :: { AbsCPP.Exp }
Exp13
  : '++' Exp14 { AbsCPP.EIncr $2 }
  | '--' Exp14 { AbsCPP.EDecr $2 }
  | '!' Exp14 { AbsCPP.ENot $2 }
  | '*' Exp14 { AbsCPP.EDeref $2 }
  | Exp14 { $1 }

Exp12 :: { AbsCPP.Exp }
Exp12
  : Exp12 '*' Exp13 { AbsCPP.ETimes $1 $3 }
  | Exp12 '/' Exp13 { AbsCPP.EDiv $1 $3 }
  | Exp12 '%' Exp13 { AbsCPP.EMod $1 $3 }
  | Exp13 { $1 }

Exp11 :: { AbsCPP.Exp }
Exp11
  : Exp11 '+' Exp12 { AbsCPP.EPlus $1 $3 }
  | Exp11 '-' Exp12 { AbsCPP.EMinus $1 $3 }
  | Exp12 { $1 }

Exp10 :: { AbsCPP.Exp }
Exp10
  : Exp10 '>>' Exp11 { AbsCPP.ERightShift $1 $3 }
  | Exp10 '<<' Exp11 { AbsCPP.ELeftShift $1 $3 }
  | Exp11 { $1 }

Exp9 :: { AbsCPP.Exp }
Exp9
  : Exp9 '<' Exp10 { AbsCPP.ELt $1 $3 }
  | Exp9 '>' Exp10 { AbsCPP.EGt $1 $3 }
  | Exp9 '<=' Exp10 { AbsCPP.ELtEq $1 $3 }
  | Exp9 '>=' Exp10 { AbsCPP.EGtEq $1 $3 }
  | Exp10 { $1 }

Exp8 :: { AbsCPP.Exp }
Exp8
  : Exp8 '==' Exp9 { AbsCPP.EEq $1 $3 }
  | Exp8 '!=' Exp9 { AbsCPP.ENEq $1 $3 }
  | Exp9 { $1 }

Exp4 :: { AbsCPP.Exp }
Exp4 : Exp4 '&&' Exp5 { AbsCPP.EAnd $1 $3 } | Exp5 { $1 }

Exp3 :: { AbsCPP.Exp }
Exp3 : Exp3 '||' Exp4 { AbsCPP.EOr $1 $3 } | Exp4 { $1 }

Exp2 :: { AbsCPP.Exp }
Exp2
  : Exp3 '=' Exp2 { AbsCPP.EAss $1 $3 }
  | Exp3 '?' Exp ':' Exp2 { AbsCPP.ECond $1 $3 $5 }
  | Exp3 { $1 }

Exp :: { AbsCPP.Exp }
Exp : Exp1 { $1 }

Exp1 :: { AbsCPP.Exp }
Exp1 : Exp2 { $1 }

Exp5 :: { AbsCPP.Exp }
Exp5 : Exp6 { $1 }

Exp6 :: { AbsCPP.Exp }
Exp6 : Exp7 { $1 }

Exp7 :: { AbsCPP.Exp }
Exp7 : Exp8 { $1 }

ListExp :: { [AbsCPP.Exp] }
ListExp
  : {- empty -} { [] }
  | Exp { (:[]) $1 }
  | Exp ',' ListExp { (:) $1 $3 }

ListId :: { [AbsCPP.Id] }
ListId : Id { (:[]) $1 } | Id ',' ListId { (:) $1 $3 }

{

type Err = Either String

happyError :: [Token] -> Err a
happyError ts = Left $
  "syntax error at " ++ tokenPos ts ++
  case ts of
    []      -> []
    [Err _] -> " due to lexer error"
    t:_     -> " before `" ++ (prToken t) ++ "'"

myLexer :: String -> [Token]
myLexer = tokens

}

