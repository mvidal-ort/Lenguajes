...
listGoodProgs = listCCFiles "tests/good"

listBadProgs = listCCFiles "tests/bad"

listCCFiles dir = 
    liftM (map (\f -> joinPath [dir,f]) . sort . filter ((=="imp") . getExt)) $ getDirectoryContents dir
...
testFrontendProg prog good f =
    do let bad = not good
       ...
       if "OK" `elem` lines out
         then if good then (return True)
              else (do reportError c "passed BAD program" f "" out err;
                       return False
                   )
         else if "TYPE ERROR" `elem` lines out
              then if good then (do reportError c "" f "" out err
                                    return False
                                )
                   else (return True)
              else (do reportError c "invalid output" f "" out err
                       return False
                   )
...
parseArgs :: [String] -> IO ()
parseArgs ["-debug"] = writeIORef doDebug True
parseArgs _ = pure ()
